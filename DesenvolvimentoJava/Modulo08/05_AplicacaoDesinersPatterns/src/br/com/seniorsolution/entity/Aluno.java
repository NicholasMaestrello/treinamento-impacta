...
public class Aluno {
	...
	public void matricular(int index) {
		this.matriculas.add(this.getCurso().getDisciplinas().get(index));
	...
	public double totalApagar() {
		...
		for (Disciplina disciplina : matriculas) {
	...
	public void realizarPagamento(TipoPagemento tipo, NomeBandeira bandeira) {
		if(tipo == TipoPagemento.Debito)
			...
			else System.out.println("Pagando com debito e cartão VISA");
		else 
			if(bandeira == NomeBandeira.MasterCard)System.out.println("Pagando com credito no cartão MasterCard");
			else System.out.println("Pagando com credito no cartão Visa");
	}
