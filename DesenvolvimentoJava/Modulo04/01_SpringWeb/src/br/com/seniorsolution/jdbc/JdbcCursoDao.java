...
public class JdbcCursoDao {
	...
	public List<Curso> listarCursos (int id) throws Exception{
		List<Curso> cursos = new ArrayList<>();
		try {
			cursos = this.jdbcTemplate.query("SELECT * FROM CURSO WHERE IDESCOLA=?",
					new Integer[] {id},
					new CursoMapper());	
		} catch (Exception e) {
	...
	public void removerCurso(int id) throws Exception{
		try {
			this.jdbcTemplate.update("DELETE FROM CURSO WHERE ID = ?", id);
		} catch (Exception e) {
			// TODO: handle exception
			throw e;
		}
