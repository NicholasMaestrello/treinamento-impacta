...
public class GenericDao<T> implements Dao<T> {
	...
	public List<T> listarRevisoes(int cod) {
		em = JpaUtil.getEntityManager();
		TypedQuery<T> query = em.createQuery("from " + classe.getSimpleName() +  " WHERE IDLIVRO = :value", classe);
		query.setParameter("value", cod);
		List<T> lista = query.getResultList();
		em.close();
		return lista;
	}
